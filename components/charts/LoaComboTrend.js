    ...
    <div style={{ position: "relative" }}>
      <svg viewBox={`0 0 ${tW} ${tH}`} width="100%" style={{ display: "block" }}>
        ...
        {months.map((m, mi) => {
          ...
          const isHover = hover === mi;
          const segs = customers.map((c) => {
            const v = c.gtgd[mi];
            const h = (v / monthMaxGT) * tIH;
            stackY -= h;
            return <rect key={c.name} x={xb} y={stackY} width={barMW} height={h} fill={c.color} opacity={isHover ? 1 : 0.9} style={{ transition: "opacity 0.15s" }} />;
          });
          return (
            <g key={mi}>
              {segs}
        ...
        {monthTotalsC.map((v, mi) => (v > 0 ? <circle key={mi} cx={lineX(mi)} cy={lineY(v)} r={hover === mi ? 5 : 3.5} fill="#fbbf24" style={{ transition: "r 0.15s" }} /> : null))}
      </svg>
      {hover != null && (
        <ChartTooltip x={(lineX(hover) / tW) * 100} y={(Math.min(tPT + tIH - (monthTotalsGT[hover] / monthMaxGT) * tIH, lineY(monthTotalsC[hover])) / tH) * 100}>
          <div style={{ fontWeight: 700, color: "#8a8fa6", fontSize: 10.5 }}>{months[hover]}</div>
          <div style={{ color: "#ecedf5", fontWeight: 800 }}>GTGD: {monthTotalsGT[hover].toFixed(1)} tỷ</div>
          <div style={{ color: "#fbbf24", fontWeight: 800 }}>Số KH: {vnInt(monthTotalsC[hover])}</div>
          {customers.map((c) => (
            <div key={c.name} style={{ color: c.color, fontSize: 10.5 }}>{c.name}: {c.gtgd[hover].toFixed(1)} tỷ</div>
          ))}
        </ChartTooltip>
      )}
    </div>
