...
export default function GroupedCompareChart({ people }) {
  ...
  return (
    <div style={{ position: "relative" }}>
                ...
                return (
                  <g key={mi}>
                    {isEmpty ? (
                      ...
                      <rect x={bx} y={by} width={barW} height={Math.max(bh, 2)} fill={m.color} rx={3} opacity={isHover ? 1 : 0.92} style={{ transition: "opacity 0.15s" }} />
                    )}
      ...
      {hover != null && (() => {
        const [gi, mi] = hover.split("-").map(Number);
        const person = people[gi];
        const m = person.metricRows[mi];
        const gx = pL + gi * groupW + (groupW - groupInnerW) / 2;
        const bx = gx + mi * (barW + barGap);
        const by = yBar(m.pct == null ? 0 : m.pct);
        return (
          <ChartTooltip x={((bx + barW / 2) / cW) * 100} y={(by / cH) * 100}>
            <div style={{ fontWeight: 700, color: "#8a8fa6", fontSize: 10.5 }}>{person.name} · {m.label}</div>
            <div style={{ color: m.color, fontWeight: 800 }}>{m.pctStr} hoàn thành KPI</div>
          </ChartTooltip>
        );
      })()}
    </div>
