      ...
      {rows.map((r, i) => (
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4, minWidth: 0 }}>
            <span style={{ fontSize: 12.5, fontWeight: 600, color: "var(--text)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.label}</span>
          ...
          <span className="mono" style={{ fontSize: 12, fontWeight: 700, color: "var(--text-strong)", whiteSpace: "nowrap" }}>{r.valueStr}</span>
        </div>
      ))}
    </div>
  );
