  ...
  return (
    <svg viewBox={`0 0 ${cW} ${cH}`} width="100%" style={{ display: "block" }}>
      ...
      {gtgd.map((v, i) => {
        ...
        return (
          <g key={"b" + i}>
            <rect x={barX(i)} y={barY(v)} width={barW} height={bh} fill={accent} opacity={0.9} rx={4} />
            {v > 0 && (
      ...
      {slgd.map((v, i) => (v > 0 ? <circle key={"d" + i} cx={lineX(i)} cy={lineY(v)} r={header ? 4 : 3.5} fill="#fbbf24" stroke="#0a0b12" strokeWidth={header ? 1.5 : 1.2} /> : null))}
      {slgd.map((v, i) => (v > 0 ? (
