...
export default function RadarHexagon({ axes, accent }) {
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {hover != null && (
        <ChartTooltip x={(axPts[hover].inner.x / 220) * 100} y={(axPts[hover].inner.y / 220) * 100}>
          <div style={{ fontWeight: 700, color: "#8a8fa6", fontSize: 10.5 }}>{axPts[hover].label}</div>
          <div style={{ color: accent, fontWeight: 800 }}>{axPts[hover].val.toFixed(0)} / 100</div>
        </ChartTooltip>
      )}
    </div>
  );
