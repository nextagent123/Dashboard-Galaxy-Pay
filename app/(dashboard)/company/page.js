...
export default function CompanyPage() {
  ...
  return (
    <>
        ...
        {companyKpi.map((k) => (
          <KpiCardShell key={k.key} label={k.label} icon={<Icon paths={METRIC_ICON_PATHS[k.key]} />} iconBg={k.iconBg} glow={k.glow}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginTop: 14, position: "relative" }}>
              ...
              <span style={{ fontSize: 13, color: "var(--text-dim)", fontWeight: 600 }}>tỷ VND / năm</span>
            </div>
            <div style={{ display: "flex", gap: 24, marginTop: 18, position: "relative", paddingTop: 14, borderTop: "1px solid var(--border-soft)" }}>
              <div>
                <div style={{ fontSize: 11, color: "var(--text-dim)" }}>H1</div>
                <div className="mono" style={{ fontSize: 15, fontWeight: 700, color: "var(--text)", marginTop: 3 }}>{k.h1} tỷ</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: "var(--text-dim)" }}>H2</div>
                <div className="mono" style={{ fontSize: 15, fontWeight: 700, color: "var(--text)", marginTop: 3 }}>{k.h2} tỷ</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: "var(--text-dim)" }}>Tháng cao nhất</div>
                <div className="mono" style={{ fontSize: 15, fontWeight: 700, color: "var(--accent-light)", marginTop: 3 }}>T{k.peakM} · {k.peakV}</div>
              </div>
            </div>
          </KpiCardShell>
        ))}
      ...
      <SectionCard title="Bảng KPI toàn công ty theo tháng" right={<div style={{ fontSize: 12, color: "var(--text-dim)" }}>Đơn vị: tỷ VND</div>}>
        <div className="table-wrap">
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5, minWidth: 820 }}>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.name}>
                  <td style={{ padding: "12px 8px", borderBottom: "1px solid var(--border-faint)", fontWeight: 700, color: "var(--text-strong)" }}>{r.name}</td>
                  {r.cells.map((c, i) => (
                    <td key={i} className="mono" style={{ padding: "12px 6px", borderBottom: "1px solid var(--border-faint)", textAlign: "right", color: c.color }}>{c.v}</td>
                  ))}
                  <td className="mono" style={{ padding: "12px 8px", borderBottom: "1px solid var(--border-faint)", textAlign: "right", fontWeight: 800, color: "var(--accent-light)" }}>{r.total}</td>
                </tr>
              ))}
            </tbody>
          </table>
