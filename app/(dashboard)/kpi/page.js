  ...
  return (
    <>
      <ReportHeader
        ...
        right={
          <div style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 12, color: "var(--text-dim)", background: "var(--surface-soft)", border: "1px solid var(--border)", padding: "9px 13px", borderRadius: 11 }}>
            ...
            <span>Số liệu đến 05/08/2026 · KPI Theo BDM</span>
          </div>
        }
      />
      ...
      <section className="card" style={{ display: "flex", flexWrap: "wrap", gap: 16, alignItems: "end", padding: "16px 20px" }}>
        <label style={{ display: "flex", flexDirection: "column", gap: 6, minWidth: 200 }}>
          <span style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 1, color: "var(--text-dim)", textTransform: "uppercase" }}>BDM phụ trách</span>
        ...
        <label style={{ display: "flex", flexDirection: "column", gap: 6, minWidth: 200 }}>
          <span style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 1, color: "var(--text-dim)", textTransform: "uppercase" }}>Thời gian đánh giá</span>
      ...
      <section style={{ background: "linear-gradient(135deg, rgba(124,108,255,0.12), rgba(124,108,255,0.03))", border: "1px solid rgba(124,108,255,0.25)", borderRadius: 14, padding: "20px 24px", display: "grid", gridTemplateColumns: "1.2fr 1fr 1fr 1fr", gap: 18, alignItems: "center" }} className="khoi-strip">
        <div>
          <div style={{ fontSize: 11, color: "var(--accent-light)", letterSpacing: 1.5, fontWeight: 700 }}>KPI KHỐI KINH DOANH · FY2026</div>
          <div style={{ fontSize: 22, fontWeight: 800, color: "var(--text-strong)", marginTop: 6 }}>Chỉ tiêu tổng khối</div>
          <div style={{ fontSize: 12, color: "var(--text-dim)", marginTop: 2 }}>Cơ sở tính đóng góp cá nhân</div>
        </div>
        <div style={{ borderLeft: "1px solid var(--border)", paddingLeft: 18 }}>
          <div style={{ fontSize: 11, color: "var(--text-dim)" }}>GMV</div>
          <div className="mono" style={{ fontSize: 22, fontWeight: 800, color: "#7c6cff" }}>{khoiTotals.gmv}</div>
        </div>
        <div style={{ borderLeft: "1px solid var(--border)", paddingLeft: 18 }}>
          <div style={{ fontSize: 11, color: "var(--text-dim)" }}>Revenue</div>
          <div className="mono" style={{ fontSize: 22, fontWeight: 800, color: "#34d399" }}>{khoiTotals.dt}</div>
        </div>
        <div style={{ borderLeft: "1px solid var(--border)", paddingLeft: 18 }}>
          <div style={{ fontSize: 11, color: "var(--text-dim)" }}>Gross Profit</div>
          <div className="mono" style={{ fontSize: 22, fontWeight: 800, color: "#f59e0b" }}>{khoiTotals.ln}</div>
        </div>
      </section>

      <section style={{ background: "var(--surface-hover)", border: "1px solid var(--border-soft)", borderRadius: 14, padding: "22px 24px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12, gap: 16, flexWrap: "wrap" }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: "var(--text-strong)" }}>Mức độ hoàn thành KPI theo cá nhân</div>
            <div style={{ fontSize: 11.5, color: "var(--text-dim)", marginTop: 3 }}>% Đạt — GMV / Revenue / Gross Profit · {periodLabel} · Mốc 100% = Đạt KPI</div>
          </div>
      ...
      {people.map((person) => (
        ...
        <section style={{ background: "linear-gradient(180deg, rgba(255,255,255,0.045), rgba(255,255,255,0.01))", border: "1px solid var(--border)", borderRadius: 18, overflow: "hidden", position: "relative" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 0, borderBottom: "1px solid var(--border-soft)" }} className="player-identity">
            <div style={{ padding: "26px 28px", display: "flex", flexDirection: "column", gap: 16, position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16, position: "relative" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6, minWidth: 0, flex: 1 }}>
                  ...
                  <div style={{ fontSize: 22, fontWeight: 900, color: "var(--text-strong)", letterSpacing: -0.3, lineHeight: 1.2 }}>{person.name}</div>
                  <div style={{ fontSize: 12.5, color: "var(--text-dim)" }}>{person.role || ""}</div>
                </div>
                <div style={{ width: 64, height: 64, borderRadius: 14, border: `2px solid ${person.accent || "#7c6cff"}`, background: "var(--surface-hover)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 800, color: person.accent || "#7c6cff", flexShrink: 0 }} className="mono">{person.short || "?"}</div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 4 }}>
                <div style={{ background: "var(--surface-hover)", border: "1px solid var(--border-soft)", borderRadius: 10, padding: "10px 12px" }}>
                  <div style={{ fontSize: 10.5, color: "var(--text-dim)", letterSpacing: 0.5, textTransform: "uppercase" }}>% Đạt bình quân</div>
                  <div className="mono" style={{ fontSize: 20, fontWeight: 800, color: "var(--text-strong)", marginTop: 2 }}>{person.overallPctStr}</div>
                </div>
                <div style={{ background: "var(--surface-hover)", border: "1px solid var(--border-soft)", borderRadius: 10, padding: "10px 12px" }}>
                  <div style={{ fontSize: 10.5, color: "var(--text-dim)", letterSpacing: 0.5, textTransform: "uppercase" }}>Đóng góp Khối</div>
                  <div className="mono" style={{ fontSize: 20, fontWeight: 800, color: person.accent || "#7c6cff", marginTop: 2 }}>{person.contribAvgStr}</div>
                </div>
              </div>
            </div>

            <div style={{ padding: "20px 28px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.15)", borderLeft: "1px solid var(--border-soft)" }}>
              <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 1.4, color: "var(--text-dim)", marginBottom: 6 }}>CHỈ SỐ TỔNG QUAN · 6 TIÊU CHÍ</div>
          ...
          <div className="grid-3" style={{ padding: "20px 24px 12px" }}>
            {person.metricRows.map((m) => (
              <div key={m.key} style={{ background: "var(--surface-hover)", border: "1px solid var(--border-soft)", borderLeft: `3px solid ${m.color}`, borderRadius: 12, padding: "14px 16px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  <div style={{ fontSize: 12, fontWeight: 800, color: m.color, letterSpacing: 0.4, textTransform: "uppercase" }}>{m.label}</div>
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "2px 8px", borderRadius: 6, background: "var(--surface-soft)", fontSize: 10.5, fontWeight: 700, color: m.statusColor }}>{m.statusIcon} {m.statusLabel}</div>
                </div>
                <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                  <span className="mono" style={{ fontSize: 22, fontWeight: 800, color: "var(--text-strong)" }}>{m.actualYTDStr}</span>
                  <span style={{ fontSize: 11, color: "var(--text-dim)" }}>/ KPI {m.kpiYTDStr}</span>
                </div>
                <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>{periodLabel}</div>
                <div style={{ position: "relative", height: 8, background: "var(--surface-soft)", borderRadius: 5, overflow: "hidden", marginTop: 10 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6, fontSize: 10.5 }}>
                  <span style={{ color: "var(--text-dim)" }}>Đóng góp Khối <span className="mono" style={{ color: "var(--text)", fontWeight: 700 }}>{m.contribStr}</span></span>
          ...
          <div style={{ padding: "0 24px 22px", display: "flex", flexDirection: "column", gap: 14 }}>
            {person.metricRows.map((m) => (
              <div key={m.key} style={{ background: "var(--surface-hover)", border: "1px solid var(--border-soft)", borderRadius: 12, padding: "14px 16px 6px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "var(--text-dim)", letterSpacing: 0.4 }}>{m.label} · Kế hoạch vs Thực tế theo tháng ({m.unit})</div>
                  <div style={{ fontSize: 10.5, color: "var(--text-dim)" }}>KPI cả năm: {m.kpiFYStr}</div>
                </div>
                <PersonMonthChart months={MONTHS_12} kpi={m.kpi} actual={m.actual} color={m.color} fmt={fmtForUnit(m.unit)} />
                <div style={{ display: "flex", gap: 12, marginTop: 2, fontSize: 9.5, color: "var(--text-dim)" }}>
      ...
      {people.length === 0 && (
        <div style={{ textAlign: "center", padding: "40px 24px", color: "var(--text-dim)", fontSize: 13.5 }}>Không tìm thấy BDM phù hợp.</div>
      )}
    </>
  );
}
