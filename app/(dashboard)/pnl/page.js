    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20, marginTop: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
        <StatTile
          ...
          value={product.totalRevenueStr + " tr"}
          sub={`Target: ${vnLoc(product.targets.revenue, 0)} tr · ${product.revPctStr}`}
        ...
        <StatTile
          label="Lợi nhuận gộp"
          value={product.totalGrossProfitStr + " tr"}
        ...
        <StatTile
          ...
          value={product.totalNetProfitStr + " tr"}
        ...
        <div style={{ fontSize: 11, color: "var(--text-faint)", marginTop: 8 }}>
          ...
          {product.targets.revenue > 0 && (
            <> · {product.revRunrate >= product.targets.revenue
              ...
              : <span style={{ color: "#fb7185" }}>Thiếu {vnLoc(product.targets.revenue - product.revRunrate, 0)} tr</span>}
            </>
          )}
        </div>
      </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20, marginTop: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12 }}>
        <StatTile label="Tổng doanh thu" value={data.consolidatedRevenueStr + " tr"} sub="Lũy kế T1–T7/2026" accent="#7c6cff" />
        <StatTile label="Tổng LN gộp" value={data.consolidatedGPStr + " tr"} sub={`Biên: ${data.consolidatedGrossMarginStr}`} accent="#fbbf24" />
        <StatTile label="Tổng LN ròng" value={data.consolidatedNPStr + " tr"} sub={`Biên: ${data.consolidatedNetMarginStr}`} accent={consolidatedNP >= 0 ? "#34d399" : "#fb7185"} />
      </div>
      ...
      <div className="card" style={{ padding: "18px 20px" }}>
        ...
        <svg viewBox="0 0 300 140" width="100%" style={{ display: "block" }}>
          {consolidatedMonthly.map((m, i) => {
            const maxR = Math.max(...consolidatedMonthly.map((m) => m.revenue));
          ...
          {(() => {
            ...
            const points = consolidatedMonthly.map((m, i) => {
              const x = i * 42 + 22;
              const y = maxR > 0 ? 110 - (m.netProfit / maxR) * 95 : 110;
              return `${x},${y}`;
            }).join(" ");
          ...
          {consolidatedMonthly.map((m, i) => {
            ...
            const x = i * 42 + 22;
            const y = maxR > 0 ? 110 - (m.netProfit / maxR) * 95 : 110;
            return <circle key={"d" + i} cx={x} cy={y} r="3" fill="#34d399" />;
          })}
        </svg>
        ...
        {VIEWS.map((v) => (
          <button
            ...
            style={{
              ...
              borderColor: view === v.value ? "rgba(124,108,255,0.3)" : "transparent",
            }}
          >
            {v.label}
          </button>
