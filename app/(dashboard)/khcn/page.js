...
const ICON = ["M17 20a4 4 0 0 0-10 0", "M12 13a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z"];

export default function KhcnPage() {
  const rpt = getSingleChannelReport("KHCN");
  const [query, setQuery] = useState("");
  const rows = getChannelDetail("KHCN", query);

  return (
    <>
      <ReportHeader
        eyebrow="GALAXY PAY · BÁO CÁO KÊNH BÁN KHCN"
        title="Báo cáo Kênh Khách hàng Cá nhân"
        subtitle={`Kênh KHCN · ${rpt.periodLabel}`}
      ...
      <section
        style={{
          ...
          background: "radial-gradient(600px 280px at 80% -10%, rgba(124,108,255,0.22), transparent 60%), linear-gradient(160deg, rgba(124,108,255,0.14), rgba(255,255,255,0.012))",
          padding: "28px 30px",
        }}
      >
        <div style={{ display: "flex", flexWrap: "wrap", gap: 28, alignItems: "center" }}>
          <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
            <ContributionRing pct={100} color={rpt.color} size={90} sub="KHCN" big />
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: 1, color: rpt.color }}>KHCN</span>
              </div>
              <div style={{ fontSize: 14.5, fontWeight: 700, color: "var(--text)", lineHeight: 1.35 }}>Khách hàng Cá nhân</div>
            </div>
          </div>
