...
export function getPersonalKpi() {
  const people = BDM.map((person) => {
    ...
    const coverage = (Math.max(...M.map((m) => m.nMonths)) / 6) * 100;
...
// BÁO CÁO DỊCH VỤ OTA
// ============================================================================
export function getOtaReport() {
  const cust = OTA_CUSTOMERS.map((c) => ({
    ...c,
    totalSlgd: sum(c.slgd),
    totalGtgd: sum(c.gtgd),
    slgdStr: sum(c.slgd).toLocaleString("vi-VN"),
    gtgdStr: sum(c.gtgd).toFixed(1) + " Tỷ",
  }));
  const grandSLGD = sum(cust.map((c) => c.totalSlgd));
  const grandGTGD = sum(cust.map((c) => c.totalGtgd));
  const avgTx = grandSLGD > 0 ? (grandGTGD * 1e9) / grandSLGD : 0;
  return {
    customers: cust,
    months: MONTHS_12,
    grandSLGD, grandGTGD,
    kpiCards: [
      { label: "Số khách hàng OTA", val: cust.length.toString(), sub: "Đang giao dịch" },
      { label: "Tổng SLGD YTD", val: (grandSLGD / 1000).toFixed(1) + "K", sub: "6 tháng đầu 2026" },
      { label: "Tổng GTGD YTD", val: grandGTGD.toFixed(1) + " Tỷ", sub: "6 tháng đầu 2026" },
      { label: "GTGD/Giao dịch", val: (avgTx / 1000).toFixed(0) + "K", sub: "Trung bình VND" },
    ],
    legendItems: cust.map((c) => ({ name: c.name, color: c.color, share: ((c.totalGtgd / grandGTGD) * 100).toFixed(1) + "%" })),
    detailRows: cust.map((c) => ({
      name: c.name, color: c.color,
      monthCells: MONTHS_12.map((_, mi) => ({ slgd: c.slgd[mi] ? c.slgd[mi].toLocaleString("vi-VN") : "", gtgd: c.gtgd[mi] ? c.gtgd[mi].toFixed(1) : "" })),
      totalSlgd: c.slgdStr, totalGtgd: c.gtgdStr,
    })),
  };
}
