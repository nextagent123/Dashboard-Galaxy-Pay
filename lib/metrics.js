...
export function getPipelineGroups(filter) {
  ...
  return groups.map((g) => {
    ...
    const totGmvContrib = sum(projects.map((p) => p.gmvContrib || 0));
    return {
      ...g,
      projects,
      appStr: vn(app, 1) + "%",
      ...
      totGmvContribStr: vnTy(totGmvContrib),
      featured: (() => {
      ...
      projRows: projects.map((p, i) => ({
        ...
        gmvContribStr: p.gmvContrib ? vnTy(p.gmvContrib) + " Tỷ" : "–",
...
export function getPnlReport() {
  const products = PNL_PRODUCTS.map((p) => {
    ...
    return {
      ...
      totalRevenueStr: vnLoc(totalRevenue, 1),
      totalGrossProfitStr: vnLoc(totalGrossProfit, 1),
      totalNetProfitStr: vnLoc(totalNetProfit, 1),
  ...
  return {
    ...
    consolidatedMonthly,
    consolidatedRevenueStr: vnLoc(consolidatedRevenue, 1),
    consolidatedGPStr: vnLoc(consolidatedGP, 1),
    consolidatedNPStr: vnLoc(consolidatedNP, 1),
