...
export function getPipelineGroups(filter) {
  ...
  return groups.map((g) => {
    ...
    const app = g.target > 0 ? (g.runrate / g.target) * 100 : 0;
    const gap = g.target - g.runrate;
    const totProj = sum(projects.map((p) => p.target || 0));
    return {
      ...g,
      projects,
      appStr: vn(app, 1) + "%",
      ...
      featured: (() => {
        ...
        const featMax = Math.max(...featured.map((f) => f.value || 0));
        return featured.map((f) => ({ ...f, barH: Math.max(35, featMax > 0 ? ((f.value || 0) / featMax) * 140 : 35) }));
      })(),
      projRows: projects.map((p, i) => ({
        ...p,
        idx: i + 1,
...
export function getPersonalKpi(period = "year") {
  ...
  const people = BDM.filter((person) => person.metrics).map((person) => {
    const M = PERSON_METRIC_DEFS.map((m) => {
      const d = person.metrics[m.key] || { unit: "tỷ", kpi: [], actual: [] };
      const kpi = d.kpi || [];
      const actual = d.actual || [];
      let kYTD = 0, aYTD = 0, nMonths = 0;
      const anyActual = actual.slice(rangeStart, rangeEnd).some((v) => v != null);
      ...
      const kFY = sum(kpi.map((v) => v || 0));
      const aBil = toBillion(aYTD, d.unit || "tỷ");
...
// ============================================================================
export const CHANNEL_LABELS = { KHCN: "Khách hàng cá nhân (KHCN)", KHDN: "Khách hàng doanh nghiệp (KHDN)" };
export const CHANNEL_COLORS = { KHCN: "#7c6cff", KHDN: "#34d399" };
...
export function getSingleChannelReport(channelKey) {
  const rows = CHANNEL_SALES.filter((r) => r.kenh === channelKey);
  const revenue = sum(rows.map((r) => r.doanhThu));
  const quantity = sum(rows.map((r) => r.soLuong));
  const commission = sum(rows.map((r) => r.hoaHong || 0));
  const uniqueCustomers = new Set(rows.map((r) => r.khachHang)).size;
  ...
  const monthlyTrend = months.map((m) => {
    const mRows = rows.filter((r) => r.thang === m);
    return {
      label: monthLabelShort(m),
      revenue: sum(mRows.map((r) => r.doanhThu)),
      quantity: sum(mRows.map((r) => r.soLuong)),
      customers: new Set(mRows.map((r) => r.khachHang)).size,
    };
  });
  ...
  const last = nonZero[nonZero.length - 1];
  const prev = nonZero[nonZero.length - 2];
  const momentum = prev && prev.revenue > 0 ? (last.revenue / prev.revenue - 1) * 100 : null;

  const topCustomers = Object.values(
    rows.reduce((acc, r) => {
      if (!acc[r.khachHang]) acc[r.khachHang] = { name: r.khachHang, revenue: 0, quantity: 0, months: new Set() };
      acc[r.khachHang].revenue += r.doanhThu;
      acc[r.khachHang].quantity += r.soLuong;
      acc[r.khachHang].months.add(r.thang);
      return acc;
    }, {})
  )
    ...
    .map((c, i) => ({
      rank: i + 1,
      name: c.name,
      revenueStr: fmtVND(c.revenue),
      revenue: c.revenue,
      quantityStr: vnInt(c.quantity),
      months: c.months.size,
      sharePct: revenue > 0 ? (c.revenue / revenue) * 100 : 0,
    }));

  return {
    key: channelKey,
    label: CHANNEL_LABELS[channelKey],
    color: CHANNEL_COLORS[channelKey],
    revenue,
    revenueStr: fmtVND(revenue),
    commissionStr: fmtVND(commission),
    avgRevenueStr: fmtVND(avgRevenue),
    ...
    momentum,
    momentumStr: momentum == null ? null : (momentum >= 0 ? "+" : "") + vn(momentum, 1) + "%",
    periodLabel: months.length > 0 ? `${monthLabelShort(months[0])} – ${monthLabelShort(months[months.length - 1])}` : "",
    peakMonth: nonZero.length > 0 ? nonZero.reduce((a, b) => (b.revenue > a.revenue ? b : a), nonZero[0]).label : "",
  };
}
...
// ============================================================================
// BÁO CÁO P&L THEO NHÓM SẢN PHẨM
// ============================================================================
const MONTH_LABELS_7 = ["T1", "T2", "T3", "T4", "T5", "T6", "T7"];

function pnlSum(arr) { return arr.reduce((a, b) => a + b, 0); }
function pnlMargin(profit, revenue) { return revenue > 0 ? (profit / revenue) * 100 : 0; }
function pnlMoM(arr, idx) {
  if (idx <= 0 || !arr[idx - 1]) return null;
  return ((arr[idx] - arr[idx - 1]) / Math.abs(arr[idx - 1])) * 100;
}

function pnlFmt(v, d) { return v >= 1 || v <= -1 ? vnLoc(v, d || 1) : vnLoc(v, d || 2); }

function processSubItem(s) {
  const m = s.monthly;
  const N = m.revenue.length;
  const totalRevenue = pnlSum(m.revenue);
  const totalGrossProfit = pnlSum(m.grossProfit);
  const totalNetProfit = pnlSum(m.netProfit);
  const totalMerchants = m.merchants ? m.merchants[N - 1] : 0;
  const grossMargin = pnlMargin(totalGrossProfit, totalRevenue);
  const netMargin = pnlMargin(totalNetProfit, totalRevenue);
  return {
    ...s, totalRevenue, totalGrossProfit, totalNetProfit, totalMerchants,
    grossMargin, netMargin,
    totalRevenueStr: pnlFmt(totalRevenue),
    totalGrossProfitStr: pnlFmt(totalGrossProfit),
    totalNetProfitStr: pnlFmt(totalNetProfit),
    grossMarginStr: vn(grossMargin, 1) + "%",
    netMarginStr: vn(netMargin, 1) + "%",
    labels: MONTH_LABELS_7,
  };
}

export function getPnlReport() {
  const products = PNL_PRODUCTS.map((p) => {
    const m = p.monthly;
    const N = m.revenue.length;
    const totalRevenue = pnlSum(m.revenue);
    const totalCogs = pnlSum(m.cogs);
    const totalGrossProfit = pnlSum(m.grossProfit);
    const totalOpex = pnlSum(m.opex);
    const totalNetProfit = pnlSum(m.netProfit);
    const totalGMV = pnlSum(m.gmv);
    const totalMerchants = m.merchants[N - 1];
    const totalTransactions = pnlSum(m.transactions);
    ...
    const revenueTarget = p.targets.revenue;
    const gpTarget = p.targets.grossProfit;
    const npTarget = p.targets.netProfit;
    const revPct = revenueTarget > 0 ? (totalRevenue / revenueTarget) * 100 : 0;
    const gpPct = gpTarget > 0 ? (totalGrossProfit / gpTarget) * 100 : 0;
    const npPct = npTarget > 0 ? (totalNetProfit / npTarget) * 100 : 0;

    const revRunrate = N > 0 ? (totalRevenue / N) * 12 : 0;
    const gpRunrate = N > 0 ? (totalGrossProfit / N) * 12 : 0;
    const npRunrate = N > 0 ? (totalNetProfit / N) * 12 : 0;

    const lastRevMoM = pnlMoM(m.revenue, N - 1);
    const lastGpMoM = pnlMoM(m.grossProfit, N - 1);

    const monthlyGrossMargin = m.revenue.map((r, i) => pnlMargin(m.grossProfit[i], r));
    const monthlyNetMargin = m.revenue.map((r, i) => pnlMargin(m.netProfit[i], r));

    const waterfall = [
      { label: "Doanh thu", value: totalRevenue, type: "revenue" },
      { label: "COGS", value: -totalCogs, type: "cost" },
      { label: "LN gộp", value: totalGrossProfit, type: "subtotal" },
      { label: "OPEX", value: -totalOpex, type: "cost" },
      { label: "LN ròng", value: totalNetProfit, type: "total" },
    ];

    // Process sub-items for expandable "Dịch vụ khác" detail
    const processedSubItems = (p.subItems || []).map(processSubItem);

    return {
      ...p,
      totalRevenue, totalCogs, totalGrossProfit, totalOpex, totalNetProfit,
      totalGMV, totalMerchants, totalTransactions,
      grossMargin, netMargin, cogsRatio, opexRatio,
      revPct, gpPct, npPct,
      revRunrate, gpRunrate, npRunrate,
      lastRevMoM, lastGpMoM,
      monthlyGrossMargin, monthlyNetMargin,
      waterfall,
      ...
      grossMarginStr: vn(grossMargin, 1) + "%",
      netMarginStr: vn(netMargin, 1) + "%",
      revPctStr: vn(revPct, 1) + "%",
    };
  });

  const consolidatedRevenue = pnlSum(products.map((p) => p.totalRevenue));
  const consolidatedGP = pnlSum(products.map((p) => p.totalGrossProfit));
  const consolidatedNP = pnlSum(products.map((p) => p.totalNetProfit));
  const consolidatedGrossMargin = pnlMargin(consolidatedGP, consolidatedRevenue);
  const consolidatedNetMargin = pnlMargin(consolidatedNP, consolidatedRevenue);

  const consolidatedMonthly = MONTH_LABELS_7.map((label, i) => ({
    label,
    revenue: pnlSum(products.map((p) => p.monthly.revenue[i])),
    grossProfit: pnlSum(products.map((p) => p.monthly.grossProfit[i])),
    netProfit: pnlSum(products.map((p) => p.monthly.netProfit[i])),
  }));
  ...
  return {
    ...
    consolidatedGP,
    consolidatedNP,
    consolidatedGrossMargin,
    consolidatedNetMargin,
    consolidatedMonthly,
    consolidatedRevenueStr: pnlFmt(consolidatedRevenue),
    consolidatedGPStr: pnlFmt(consolidatedGP),
    consolidatedNPStr: pnlFmt(consolidatedNP),
    consolidatedGrossMarginStr: vn(consolidatedGrossMargin, 1) + "%",
    consolidatedNetMarginStr: vn(consolidatedNetMargin, 1) + "%",
  };
}
